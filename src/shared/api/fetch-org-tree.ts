...
import type { OrgGraph } from '@/entities/org-node/model/types';
...
export async function fetchOrgTree({ signal }: { signal: AbortSignal }): Promise<OrgTreeResponse> {
  ...
  const parsed = orgTreeSchema.safeParse(await response.json());
  ...
  try { return { graph: createOrgGraph(parsed.data), revision }; } catch (error) {
